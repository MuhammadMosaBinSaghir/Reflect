...
final class Records {
    ...
    func pull(from url: URL) {
        ...
        statements.append(.init(name: name, type: type, theme: .BMO, data: data))
        ...
        Regex {
            ...
            Capture { cardRegex }
            seperatorRegex
            Capture(as: account) { accountRegex }
            seperatorRegex
            Capture(as: date) { dateRegex }
            seperatorRegex
            Capture(as: amount) { amountRegex }
            seperatorRegex
            Capture { codeRegex }
            Capture { descriptionRegex }
         }
        .anchorsMatchLineEndings()
        
        let transactions = data.matches(of: transactionRegex) // 2
        print("Found \(transactions.count) matches")
        for transaction in transactions { // 3
            print("full: " + transaction.output.0)
            print("card: " + transaction[card])
            print("account: " + transaction[account])
            print("date: " + transaction[date])
            print("amount: " + transaction[amount])
            print("code: " + transaction[code])
